...
public class BinaryOnRibs {
    ...
    static int getMin(int u, int v){
        ...
        for (int i = h; i >= 0; i--) {
            if (dp[u][i] != dp[v][i]){
                res = Math.min(Math.min(min[u][i], min[v][i]), res);
                u = dp[u][i];
                v = dp[v][i];
            }
        }
        return Math.min(Math.min(min[u][0], min[v][0]), res);
    }
    public static void main(String[] args) {
        ...
        for (int i = 1; i < h + 1; i++){
            for (int v = 1; v < n + 1; v++){
                min[v][i] = Math.min(min[dp[v][i - 1]][i - 1], min[v][i - 1]);
            }
        }
        int m = input.nextInt();
        for (int i = 0; i < m; i++) {
            out.println(getMin(input.nextInt(), input.nextInt()));
        }
/*
        for (int i = 1; i < n + 1; i++) {
